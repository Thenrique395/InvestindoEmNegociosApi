...
public sealed class CsrfValidationMiddleware(RequestDelegate next)
{
    ...
    public async Task InvokeAsync(HttpContext context)
    {
        ...
        // Webhooks de gateway de pagamento (Stripe/Mercado Pago) não têm sessão de browser,
        // logo não têm o cookie XSRF-TOKEN — a checagem abaixo só bloqueia quando o cookie
        // existe (sessão autenticada via browser), então requisições de webhook passam direto.
