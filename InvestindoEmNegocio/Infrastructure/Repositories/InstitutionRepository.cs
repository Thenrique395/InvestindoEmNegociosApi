...
public class InstitutionRepository(InvestDbContext context) : IInstitutionRepository
{
    public async Task<List<Institution>> ListActiveAsync(InstitutionType? type = null, CancellationToken cancellationToken = default)
    {
        var query = context.Institutions.AsNoTracking().Where(i => i.IsActive);
        if (type.HasValue)
    ...
    {
        return await context.Institutions.FirstOrDefaultAsync(i => i.Id == id, cancellationToken);
    }
